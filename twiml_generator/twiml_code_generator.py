...
class TwimlCodeGenerator(object):
    ...
    def __init__(
        self, twiml_filepath, code_filepath=None, lib_filepath=None, language="python"
    ):
    ...
    def get_code_filepath(self):
        ...
        code_filepath = language_dirpath / filepath.name.replace(
            ".xml", self.language_spec["extension"]
        )
    ...
    def output_imports(self):
        ...
        if self.language_spec.get("add_imports") == "single_line":
            imports = [
                verb if verb != "Response" else self.class_for_verb_name(verb)
                for verb in imports
            ]
            return (
                self.language_spec[import_kind].format(imports=", ".join(imports))
                + "\n"
            )
            ...
            for class_name in classes_to_import:
                if self.language_spec.get(
                    "import_common"
                ) and class_name in self.language_spec.get("common_classes", []):
                    imports.append(
                        self.language_spec["import_common"].format(imports=class_name)
                    )
                else:
                    imports.append(
                        self.language_spec[import_kind].format(imports=class_name)
                    )
            imports = "\n".join(imports) + "\n"
    ...
    def output_padded_code(self, lines):
        ...
        return (
            "\n".join(
                [
                    " " * self.language_spec["code_wrapper_padding"] + line
                    for line in lines
                ]
            )
            + "\n"
    ...
    def join_attributes_for_verb(self, verb):
        ...
        if len(verb.attributes) > 0:
            ...
            joined_attributes = self.language_spec.get("attribute_join", ", ").join(
                attributes
            )
            return self.language_spec.get(
    ...
    def build_attributes_for_verb(self, verb):
        ...
        for name, value in verb.attributes.items():
            ...
            if (
                self.language_spec.get("attributes_map")
                ...
                if self.language_spec["attributes_map"][name].get("_import"):
                    self.specific_imports.add(
                        self.language_spec["attributes_map"][name]["_import"]
                    )
            ...
            built_attributes.append(
                self.language_spec["attribute_format"].format(name=name, value=value)
            )
    ...
    def format_code(self):
        if "formatter" not in self.language_spec:
            return
        format_cmd = self.language_spec["formatter"].format(
            filepath=str(self.code_filepath)
        )
        subprocess.run(
            [format_cmd],
            shell=True,
    ...
    def verify(self):
        ...
        if p.returncode == 0:
            ...
            return (
                TwimlCodeGenerator.VERIFY_SUCCESS
                if comparison_result
                else TwimlCodeGenerator.VERIFY_FAILURE,
                "\n".join([p.stdout.decode(), p.stderr.decode()]),
                etree.tostring(
                    input_tree, encoding="utf-8", pretty_print=True
                ).decode(),
                etree.tostring(
                    output_tree, encoding="utf-8", pretty_print=True
                ).decode(),
            )
        else:
            return (
    ...
    def verify_generic(self):
        ...
        return subprocess.run(
            generic_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )

    def verify_java(self):
        if not shutil.which("java"):
            raise Exception(
                "You need to install java if you want to verify a java file"
            )
        ...
        p = subprocess.run(
            javac_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        if p.returncode != 0:
            java_cleanup()
            return p
    ...
    def verify_csharp(self):
        if not shutil.which("dotnet"):
            raise Exception(
                "You need to install dotnet core if you want to verify a C# file"
            )
        ...
        if is_new_env:
            ...
            p = subprocess.run(
                dotnet_new_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            if p.returncode != 0:
                csharp_clean()
                return p
        ...
        p = subprocess.run(
            dotnet_run_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        csharp_clean()
        return p
