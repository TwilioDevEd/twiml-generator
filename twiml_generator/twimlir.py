...
class TwimlAttributesTypes(dict):
    ...
    def get_xsd_filepath(self, xsd_filepath):
        ...
        if isinstance(xsd_filepath, Path):
            ...
            return (
                ...
                / "xsd"
                / "twiml_v20100401_twilioRealtimeCallSchema.xsd"
            )
        else:
    ...
    def build_attributes_types(self):
        ...
        for complexType in tree.iter("{http://www.w3.org/2001/XMLSchema}complexType"):
            ...
            attributes_types = {}
            for attribute in complexType.iter(
...
class TwimlIR(object):
    ...
    def parse_xml(self):
        ...
        for event, twiml_verb in etree.iterparse(
            str(self.xml_filepath), events=("start", "end")
        ):
            if event == "start":
                ...
                if latest_verb:
                    logger.debug(
                        "Set Latest Verb Seen as : {}".format(latest_verb.name)
                    )
                if twiml_verb.tag == "Say" and len(list(twiml_verb)) > 0:
                    processing_ssml = False
    ...
    def __iter__(self):
        ...
        while queue:
            ...
            if event == "start":
                ...
                queue.extend(
                    [
                        (v, "start") if not v.is_leaf else (v, "leaf")
    ...
    def reverse_iter(self):
        ...
        while queue:
            ...
            if verb in visited:
                continue
            elif verb.children and len(set(verb.children) & visited) < len(
                verb.children
            ):
...
class TwimlIRVerb(object):
    ...
    def __repr__(self):
        ...
        return "<{name}{attributes_string}>{text_string}".format(
            name=self.name, attributes_string=attributes_string, text_string=text_string
        )

    def add_child(self, name, text):
        newVerb = TwimlIRVerb(name=name, attributes={}, text=text, parent=self)
