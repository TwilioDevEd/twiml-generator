...
from twiml_generator.specificity.common import (
    Language,
    attr_to_list,
    rename_attr,
    to_bytes,
...
def to_list(verb, attr_name, imports, **kwargs):
    if (
        attr_to_list(verb, attr_name, formatter="Arrays.asList({})", **kwargs)
...
def enum_name(verb_name, attr_name, value):
    return ".".join(
        [verb_name, camelize(attr_name), underscore(value).upper().replace(".", "_")]
    )
...
def to_enum(verb, attr_name):
    if attr_name in verb.attributes and not isinstance(
        ...
        verb.attributes[attr_name] = enum_name(
            verb.name, attr_name, verb.attributes[attr_name]
        )
        ...
        for verb, event in generator.twimlir:
            if verb.is_ssml:
                verb.variable_name = camelize(
                    "ssml_" + verb.name, uppercase_first_letter=False
                )
...
class Prompt:
    ...
    def process(cls, verb, imports):
        ...
        to_list(verb, "cardType", imports, transform=enum_builder(verb, "cardType"))
        to_bytes(verb, "cardType")
        rename_attr(verb, "cardType", "cardTypes")

        to_list(verb, "errorType", imports, transform=enum_builder(verb, "errorType"))
