...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l",
        "--language",
        help="Language for the code to generate",
        choices=["csharp", "java", "node", "php", "python", "ruby"],
    )
    parser.add_argument("-out", "--outpath", help="[optional] Path to output file")
    parser.add_argument(
        "--verify", action="store_false", help="Only runs the verification"
    )
    args = parser.parse_args()

    code_generator = TwimlCodeGenerator(
        args.twiml_filepath, code_filepath=args.outpath, language=args.language
    )
    if args.verify:
